...
def gallery():
    ...
    for i,p in enumerate(D['gallery']):
        ...
        photos+=f'''<button class="gallery-photo" data-photo="{p['file']}" data-caption="ATG community · Photo {i+1} of 12" aria-label="Open ATG community photo {i+1}">{image(p,f'ATG members at a club gathering, photo {i+1}',extra=f'style="object-position:{pos}"')}<span>View photo <span>↗</span></span></button>'''
...
def placement():
    ...
    tabs+='</div>'
    return subhero('Placement / Our alumni network','Ambition meets<br><em>opportunity.</em>','Our members go on to work at leading firms in finance and beyond. An enduring alumni network connects every new class with the experience of those who came before.',D['placement']['club_photo'],'ATG members together in New York')+f'''
